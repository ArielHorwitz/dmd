...
APP_NAME=$(basename "$0")
ABOUT="Pick an emoji via fuzzel and load into clipboard"
CLI=(
    --prefix "args_"
...
selection=$(jq -r "$jq_filter" < "$EMOJIS_DATA" | fuzzel --dmenu --mesg "Pick an emoji and copy to clipboard")
[[ -n $selection ]] || exit 0

emoji=$(printf '%s' "$selection" | awk '{ printf "%s", $1 }')
